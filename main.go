...
func init() {
	...
	db.AutoMigrate(&urlCheckModel{})
	db.AutoMigrate(&urlStatusModel{})
}
...
func checkAllurl(url1 string, crawlTime int, frequency int, failureThreshold int) {
	...
	for ; i <= failureThreshold; i++ {
		...
		if err != nil {
			fmt.Println("ao", i)
			urlStat := urlStatusModel{URL1: url1, AttemptNo: i, Stat: false}
			db.Save(&urlStat)
			//c.JSON(http.StatusNotFound, gin.H{"status": http.StatusNotFound, "error": err})
		} else {
			fmt.Println("bo", i)
			urlStat := urlStatusModel{URL1: url1, AttemptNo: i, Stat: true}
			db.Save(&urlStat)
...
func fetchSingleurlCheck(c *gin.Context) {
	...
	for ; i <= urlCheck.FailureThreshold; i++ {
		...
		if err != nil {
			//fmt.Println("yo2")
			urlStat := urlStatusModel{URL1: urlCheck.URL1, AttemptNo: i, Stat: false}
			db.Save(&urlStat)
			c.JSON(http.StatusNotFound, gin.H{"status": http.StatusNotFound, "error": err})
		} else {
			urlStat := urlStatusModel{URL1: urlCheck.URL1, AttemptNo: i, Stat: true}
			db.Save(&urlStat)
			c.JSON(http.StatusOK, gin.H{"status": http.StatusOK, "HTTP Response Status": resp.StatusCode, "error": err})
			break
		}
...
func main() {
	...
	{
		v1.POST("/", createurlCheck)
		v1.GET("/", fetchAllurlCheck)
		v1.GET("/:id", fetchSingleurlCheck)
		v1.PUT("/:id", updateurlCheck)
		v1.DELETE("/:id", deleteurlCheck)
	}
	router.Run()
